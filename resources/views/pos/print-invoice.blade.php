            ...
            <tbody>
                @foreach ($transaksi->transaksiItem as $item)
                    <tr>
                        ...
                        <td>
                            @foreach ($item->spesifikasi as $specId => $spec)
                                {{ \App\Models\SpesifikasiProduk::find($specId)->spesifikasi->nama_spesifikasi }}:
                        ...
                        <td>Rp
                            {{ number_format(
                                $item->harga_satuan * $item->kuantitas +
                                    collect($item->spesifikasi)->sum(function ($spec) use ($item) {
                                        return $spec['harga_per_satuan'] * $item->kuantitas;
                                    }),
                                0,
                                ',',
                                '.',
